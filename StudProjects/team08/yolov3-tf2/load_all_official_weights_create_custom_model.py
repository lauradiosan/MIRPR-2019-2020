...
def copy_darknet_weights_to_custom_model(original_model, custom_model, sub_model_layer_name='yolo_darknet'):
    ...
    if FLAGS.tiny:
        convolution_layers_names = ['yolo_conv_0', 'yolo_conv_1']
        for layer in convolution_layers_names:
            sub_layer = original_model.get_layer(layer)
            custom_model.get_layer(layer).set_weights(sub_layer.get_weights())

    else:
        convolution_layers_names = ['yolo_conv_0', 'yolo_conv_1', 'yolo_conv_2']
        for layer in convolution_layers_names:
            sub_layer = original_model.get_layer(layer)
            custom_model.get_layer(layer).set_weights(sub_layer.get_weights())

...
class LunarLanderCustom(gym.Env, EzPickle):
    ...
    def step(self, action):
        ...
        if self.game_over or abs(state[0]) >= 1.0:
            ...
            reward = -100
            position = np.array(self.lander.position)
            # reward -= sum(np.square(position)) ** 0.55
        ...
        # print(sum(np.square(position))**0.55,'is the sum')
...
class LunarLanderContinuous(LunarLanderCustom):
    continuous = True
...
if __name__ == '__main__':
    demo_heuristic_lander(LunarLanderCustom(), render=True)
